param(
    [string]$OutputRoot = "Assets/Art/Source",
    [string]$ManifestPath = "Assets/Art/Source/art_manifest.json",
    [int]$IconWidth = 512,
    [int]$IconHeight = 512,
    [int]$SceneWidth = 1920,
...
function Get-TextSourceFiles {
    ...
    foreach ($root in $roots) {
        ...
        Get-ChildItem -Path $root -Recurse -File -Include *.cs, *.md, *.json, *.txt | ForEach-Object {
            if ($_.FullName -match "[\\/]Assets[\\/]Art[\\/]Source[\\/]") {
                return
            }
...
function Build-FileName {
    ...
    if ($IsScene) {
        $sceneName = $Id -replace '^scene_', ''
        return "{0}_v01.png" -f $sceneName
    }

    if ($Id -match "_v[0-9]{2}$") {
        return "{0}.png" -f $Id
    }

    return "{0}_v01.png" -f $Id
}
...
if ($iconIds.Count -eq 0 -and $sceneIds.Count -eq 0) {
    throw "No source-art targets were discovered."
}
...
foreach ($id in $iconIds) {
    ...
    New-SourceArtPng -Path $filePath -Id $id -ImageWidth $IconWidth -ImageHeight $IconHeight
    $iconCount++
...
foreach ($sceneId in $sceneIds) {
    ...
    $fileName = Build-FileName -Id $sceneId -IsScene $true
    New-Item -ItemType Directory -Path $sceneOutputRoot -Force | Out-Null
    $filePath = Join-Path $sceneOutputRoot $fileName

    New-SourceArtPng -Path $filePath -Id $sceneId -ImageWidth $SceneWidth -ImageHeight $SceneHeight
    $sceneCount++
...
Write-Host ("Generated {0} source-art PNG(s): {1} icon(s), {2} scene image(s)." -f $manifestEntries.Count, $iconCount, $sceneCount)
